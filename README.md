...
- **40+ checks** spanning meta tags, hreflang, FAQ schema, HowTo schema, Organization schema, retrieval-crawler accessibility (`OAI-SearchBot`, `Claude-SearchBot`, `PerplexityBot`), training vs retrieval vs user-fetch bot separation, on-domain answer coverage for `site:`-scoped fanouts, first-party vs off-domain content risk, factual product summaries, machine-readable specifications, and policy schema.
